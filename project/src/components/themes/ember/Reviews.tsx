...
export default function Reviews() {
  ...
  return (
    <section id="reviews" className="py-32 px-6 bg-[#1a1a1a]">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          {reviewsData.sectionSubtitle && (
            <p className="text-xl text-gray-400">{t(reviewsData, 'sectionSubtitle')}</p>
          )}
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviewsData.reviews.map((review, index) => (
            <motion.div
              ...
              whileHover={{ y: -6 }}
              className="bg-[#0a0a0a] rounded-3xl border border-[#D4AF37]/20 hover:border-[#D4AF37]/40 p-8 transition-colors"
            >
              {/* Stars */}
              <div className="flex gap-1 mb-6">
                {Array.from({ length: review.rating || 5 }).map((_, i) => (
                  <Star key={i} className="w-5 h-5 fill-blue-500 text-[#D4AF37]" />
                ))}
              </div>
              ...
              <div className="flex items-center gap-4">
                {review.avatar ? (
                  ...
                  <div className="w-12 h-12 rounded-full bg-[#8B0000]/20 border-2 border-[#D4AF37]/40 flex items-center justify-center">
                    <span className="text-[#D4AF37] font-serif font-bold tracking-wide text-lg">{(t(review, 'name') || '?')[0]}</span>
                  </div>
                )}
                <div>
