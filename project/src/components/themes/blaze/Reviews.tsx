        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {reviewsData.reviews.map((review, index) => (
            <motion.div
              ...
              whileHover={{ y: -6 }}
              className="bg-[#0a0a0a] rounded-3xl border border-[#F59E0B]/20 hover:border-[#F59E0B]/40 p-8 transition-colors"
            >
              {/* Stars */}
              <div className="flex gap-1 mb-6">
                {Array.from({ length: review.rating || 5 }).map((_, i) => (
                  <Star key={i} className="w-5 h-5 fill-blue-500 text-[#F59E0B]" />
                ))}
              </div>
