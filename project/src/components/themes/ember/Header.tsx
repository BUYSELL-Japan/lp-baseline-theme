  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${
        scrolled 
          ? 'py-3 bg-[#0a0a0a]/95 backdrop-blur-xl border-b border-[#D4AF37]/20 shadow-2xl' 
          : 'py-6 bg-transparent'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between gap-4">
        ...
        >
          ...
          <div className="text-sm sm:text-xl font-serif font-bold tracking-wide text-white break-words leading-tight">
            {logoText}
          </div>
        </motion.div>
        ...
        <nav className="hidden xl:flex flex-wrap items-center justify-end gap-x-4 gap-y-2 max-w-full">
          {navigation.map((item) => (
            <button
              ...
              className="text-xs font-serif font-bold uppercase tracking-widest text-gray-400 hover:text-[#D4AF37] transition-colors whitespace-nowrap"
            >
        ...
        <div className="flex items-center gap-2 text-gray-400">
          ...
          <select 
            ...
            className="bg-transparent text-xs font-serif font-bold uppercase tracking-tight focus:outline-none cursor-pointer hover:text-white"
          >
        ...
        {/* Mobile Toggle */}
        <button 
          className="xl:hidden w-10 h-10 flex items-center justify-center text-white hover:bg-[#141414]/10 rounded-lg transition-colors"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            className="fixed inset-0 bg-[#0a0a0a] z-[110] flex flex-col pt-20 xl:hidden"
          >
            {/* 閉じるボタン — メニュー内右上 */}
            <button
              onClick={() => setMobileMenuOpen(false)}
              className="absolute top-5 right-5 p-2 text-white hover:bg-[#141414]/10 rounded-lg transition-colors"
            ...
            <div className="px-8 py-6 border-t border-[#D4AF37]/20">
              <p className="text-xs font-serif font-bold text-gray-200 uppercase tracking-widest mb-3">{translate('languageLabel', language)}</p>
              <div className="grid grid-cols-2 gap-2">
                {Object.keys(languageNames).map(lang => (
                  <a 
                    key={lang} 
                    href={`${basePath}${lang === 'ja' ? '' : lang + '/'}`}
                    className={`px-4 py-3 rounded-xl font-serif font-bold text-center text-sm transition-colors ${language === lang ? 'bg-[#8B0000] text-white' : 'bg-[#2a2a2a] text-gray-300 hover:bg-[#222222]'}`}
                  >
