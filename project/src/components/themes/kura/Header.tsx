  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${
        scrolled 
          ? 'py-3 bg-[#FFFFFF]/95 backdrop-blur-xl border-b border-[#C0392B]/20 shadow-2xl' 
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between gap-4">
        ...
        <nav className="hidden xl:flex flex-wrap items-center justify-end gap-x-4 gap-y-2 max-w-full">
          {navigation.map((item) => (
            <button
              ...
              className="text-xs font-serif tracking-wider uppercase tracking-widest text-amber-900/80 hover:text-[#C0392B] transition-colors whitespace-nowrap"
            >
        ...
        <div className="flex items-center gap-2 text-amber-900/80">
          ...
          <select 
            ...
            className="bg-transparent text-xs font-serif tracking-wider uppercase tracking-tight focus:outline-none cursor-pointer hover:text-[#2D2D2D]"
          >
        ...
        {/* Mobile Toggle */}
        <button 
          className="xl:hidden w-10 h-10 flex items-center justify-center text-[#2D2D2D] hover:bg-[#1a1a1a]/10 rounded-lg transition-colors"
        ...
        {mobileMenuOpen && (
          ...
          >
            ...
            <button
              ...
              className="absolute top-5 right-5 p-2 text-[#2D2D2D] hover:bg-[#1a1a1a]/10 rounded-lg transition-colors"
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6 space-y-2">
              {navigation.map((item, index) => (
                <motion.button
                  key={item.id}
                  onClick={() => scrollToSection(item.id)}
                  className="block w-full text-left py-4 text-2xl font-serif tracking-widest text-[#2D2D2D] hover:text-[#C0392B] transition-colors border-b border-[#C0392B]/20"
            ...
            <div className="px-8 py-6 border-t border-[#C0392B]/20">
              <p className="text-xs font-serif tracking-wider text-[#2D2D2D] uppercase tracking-widest mb-3">{translate('languageLabel', language)}</p>
              <div className="grid grid-cols-2 gap-2">
                {Object.keys(languageNames).map(lang => (
                  <a 
                    key={lang} 
                    href={`${basePath}${lang === 'ja' ? '' : lang + '/'}`}
                    className={`px-4 py-3 rounded-xl font-serif tracking-wider text-center text-sm transition-colors ${language === lang ? 'bg-[#1a1a1a] text-white' : 'bg-[#F5F0E8] text-[#1a1a1a] hover:bg-[#FFFFFF]'}`}
                  >
