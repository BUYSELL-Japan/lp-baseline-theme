...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="scroll-mt-20 py-16 sm:py-32 md:py-48 md:py-28 lg:py-32 px-4 sm:px-6 bg-white">
        ...
        <div className="space-y-3">
          {faqData.items.map((item, index) => (
            <motion.div
              ...
              className="bg-white rounded-2xl border border-[#C0392B]/30 overflow-hidden hover:border-[#C0392B] transition-colors"
            >
              <button
                onClick={() => setOpenIndex(openIndex === index ? null : index)}
                className="w-full p-6 flex items-center justify-between text-left"
              >
                <h3 className="text-lg font-serif tracking-widest text-[#2D2D2D] tracking-tight pr-4">
