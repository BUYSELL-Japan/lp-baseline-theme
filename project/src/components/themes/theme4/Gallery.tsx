        ...
        <div className="flex flex-wrap gap-4 mb-20 justify-start">
          <button
            ...
            className={`px-10 py-4 rounded-full font-black uppercase tracking-widest text-xs transition-all border-4 ${
              selectedCategoryIndex === 0 ? 'bg-red-600 text-yellow-400 border-red-600 shadow-2xl' : 'text-red-900 border-red-900/10 hover:border-red-600'
            }`}
          >
            {allCategory}
          </button>
          {categories.map((cat, idx) => (
            <button
              ...
              className={`px-10 py-4 rounded-full font-black uppercase tracking-widest text-xs transition-all border-4 ${
                selectedCategoryIndex === idx + 1 ? 'bg-red-600 text-yellow-400 border-red-600 shadow-2xl' : 'text-red-900 border-red-900/10 hover:border-red-600'
              }`}
            >
              {getText(cat)}
            </button>
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-red-600/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-8">
                  <p className="text-yellow-400 font-black text-2xl tracking-tighter italic">{getText(image.caption)}</p>
                </div>
              </motion.div>
            );
          })}
