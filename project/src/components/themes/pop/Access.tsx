      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="bg-white rounded-3xl border border-[#FFBE0B]/30 p-8 space-y-6"
          >
            {accessData.address && (
              <div className="flex items-start gap-4">
                <div className="bg-[#FFFFFF] border border-[#FFBE0B] p-3 rounded-xl flex-shrink-0">
                  
                </div>
                <div>
                  <h3 className="font-sans font-black tracking-tight text-[#2D2D2D] mb-1">{translate('address', language)}</h3>
            ...
            {hasParking && (
              <div className="border-t border-[#FFBE0B]/30 pt-6 flex items-start gap-4">
                <div className="bg-[#FFFFFF] border border-[#FFBE0B] p-3 rounded-xl flex-shrink-0">
                  
                </div>
                <div>
                  <h3 className="font-sans font-black tracking-tight text-[#2D2D2D] mb-1">{getText(accessData.parking.title)}</h3>
                  ...
                  {accessData.parking.spaces && (
                    <p className="text-[#FB5607] font-sans font-black tracking-tight text-sm">
            ...
            {hasTransportation && (
              <div className="border-t border-[#FFBE0B]/30 pt-6 flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-sans font-black tracking-tight text-[#2D2D2D] mb-3">{getText(accessData.transportation.title)}</h3>
                    ...
                    {accessData.transportation.methods && Array.isArray(accessData.transportation.methods) &&
                      accessData.transportation.methods.map((method, index) => (
                        <div key={index} className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-[#7B2FBE] rounded-full mt-2 flex-shrink-0" />
                          <div>
                            <span className="font-sans font-black tracking-tight text-[#2D2D2D] text-sm">{getText(method.type)}:</span>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="rounded-3xl overflow-hidden border border-[#FFBE0B]/30"
          >
